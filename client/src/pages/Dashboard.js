...
const Dashboard = () => {
  ...
  const getUserData = async () => {
    const existingUserData = await axios.get(
      `/api/user/get/gId/${userData.googleId}`
    );
  ...
  useEffect(() => {
    if (userByEmail) {
      ...
      if (userByEmail.socialMedia) {
        userByEmail.socialMedia.instagram
          ? setinstagram(userByEmail.socialMedia.instagram)
          : setinstagram("");
        userByEmail.socialMedia.twitter
          ? settwitter(userByEmail.socialMedia.twitter)
          : settwitter("");
        userByEmail.socialMedia.github
          ? setgithub(userByEmail.socialMedia.github)
          : setgithub("");
        userByEmail.socialMedia.linkedin
          ? setlinkedin(userByEmail.socialMedia.linkedin)
          : setlinkedin("");
        userByEmail.socialMedia.facebook
          ? setfacebook(userByEmail.socialMedia.facebook)
          : setfacebook("");
        userByEmail.socialMedia.discord
          ? setdiscord(userByEmail.socialMedia.discord)
          : setdiscord("");
        userByEmail.socialMedia.youtube
          ? setyoutube(userByEmail.socialMedia.youtube)
          : setyoutube("");
        userByEmail.socialMedia.blogs
          ? setblogs(userByEmail.socialMedia.blogs)
          : setblogs("");
        userByEmail.socialMedia.website
          ? setwebsite(userByEmail.socialMedia.website)
          : setwebsite("");
        userByEmail.socialMedia.telegram
          ? settelegram(userByEmail.socialMedia.telegram)
          : settelegram("");
        userByEmail.socialMedia.pintrest
          ? setpintrest(userByEmail.socialMedia.pintrest)
          : setpintrest("");
        userByEmail.socialMedia.buymeacoffee
          ? setbuymeacoffee(userByEmail.socialMedia.buymeacoffee)
          : setbuymeacoffee("");
        userByEmail.bio ? setBio(userByEmail.bio) : setBio("");
        userByEmail.profession
          ? setProfession(userByEmail.profession)
          : setProfession("");
      }
      setLoading(false);
    }
  ...
  const onSubmit = async (event) => {
    ...
    if (
      ...
      telegram.includes(" ") ||
      pintrest.includes(" ") ||
      buymeacoffee.includes(" ")
    ) {
      notify(
        "Spaces included links will be not saved, Please remove spaces from links.",
        "error"
      );
    }
    socialMediaArr.forEach((platformName) => {
      if (!platformName.includes("https://") && platformName !== "") {
        notify(
          `'${platformName}' must include https (Update with proper link)`,
          "error"
        );
        submitForm(false);
      }
    ...
    if (submitForm) {
      ...
      // console.log(availableUsername.data.data[0].googleId, userData.googleId);
      if (
        availableUsername.data.results > 0 &&
        availableUsername.data.data[0].googleId !== userData.googleId
      ) {
  ...
  return (
    <>
      <Helmet>
        <title>Dashboard || LinksX.Pro</title>
      </Helmet>
      ...
      {userByEmail && !loading && (
        <div className="container px-10 md:px-32 lg:px-[24rem] my-20">
          ...
          <img
            className="w-32 mx-auto my-5 h-auto rounded-full shadow-sm"
            src={
              userData.imageUrl &&
              userData.imageUrl.replace("w=s96-c", "w=s240-c")
            }
            alt={userData.name}
            referrerPolicy="no-referrer"
          />
          <div>
            <h2 className="my-2 text-5xl text-center font-bold text-gray-800">
              Dashboard
            </h2>
            <hr className="my-5" />
          </div>
          ...
          <form onSubmit={onSubmit} autoComplete="off">
            <div>
              <label
                htmlFor="input-label"
                className="block text-sm font-medium ml-1 -mb-2 mt-1 dark:text-white"
              >
                Name
              </label>
              <input
                ...
                required
                name="fullName"
                className="py-3 px-4 block w-full rounded-md input-style text-md my-4"
                readOnly
              />
              <label
                htmlFor="input-label"
              ...
              <label
                htmlFor="input-label"
                className="block text-sm ml-1 mb-4 -mt-2 text-green-500"
              >
              ...
              <textarea
                ...
                name="bio"
                required
                className="py-3 px-4 block w-full rounded-md text-md input-style my-4 border-gray-300 border border-solid"
                rows="3"
            ...
            <p className="text-red-500 text-sm">
              All fields aren't required but still fill some links for better
              profile.
            </p>
            <div>
              <div>
                ...
                <InputLink
                  ...
                  placeholder="https://www.linkedin/username"
                />
                <InputLink
                  platform="facebook"
                  iconName="FiFacebook"
                ...
                <InputLink
                  platform="discord"
                  iconName="TbBrandDiscord"
                  placeholder="https://www.discordapp.com/users/xxxx"
                />
                <InputLink
                  platform="youtube"
                  iconName="FiYoutube"
                ...
                <InputLink
                  platform="blogs"
                  iconName="ImBlog"
                  placeholder="Medium, Hashnode, Dev Community etc"
                />
                <InputLink
                  platform="website"
                  iconName="TbWorld"
                  placeholder="Personal or any company website"
                />
                <InputLink
                  platform="telegram"
                  iconName="TbBrandTelegram"
                  placeholder="https://www.t.me/username"
                />
                <InputLink
                  platform="pintrest"
                  iconName="ImPinterest2"
                  placeholder="https://www.pintrest.com/username"
                />
                <InputLink
            ...
            <input
              type="submit"
              className="bg-blueColor py-3 px-4 my-10 text-white rounded-md focus:ring-2 duration-200 cursor-pointer ring-offset-2 ring-blueColor/30"
